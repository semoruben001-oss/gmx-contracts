...
async function splitAndDistributeWnt(isManual = false) {
  ...
  if (balance.lt(hre.ethers.utils.parseUnits(SEND_THRESHOLD, wntTokenInfo.decimals))) {
    throw new Error(`amount should be greater than threshold (${SEND_THRESHOLD} WNT)`);
  }
  const amount = getAmountToDistribute(amountInput, wntTokenInfo.decimals, balance);

  if (amount.lte(0)) {
    throw new Error("amount to send should be greater than 0");
  }
...
module.exports = { splitAndDistributeWnt };
